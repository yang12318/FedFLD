...
class DatasetObject:
    def __init__(self, dataset, n_client, seed, rule, rule_arg='', data_path=''):
        ...
        self.clnt_x, self.clnt_y, self.tst_x, self.tst_y = self._load_split_data(seed, rule, rule_arg)
    ...
    def _split_data(self, clnt_data_list, trn_x, trn_y, rule, rule_arg):
        if rule == 'Dirichlet':
    ...
    def _load_split_data(self, seed, rule, rule_arg):
        ...
        if not os.path.exists('%sData/%s' % (self.data_path, self.name)):
            ...
            clnt_data_list = (
                np.random.lognormal(mean=np.log(n_data_per_clnt), size=self.n_client))
            ...
            clnt_x, clnt_y = self._split_data(clnt_data_list, trn_x, trn_y, rule, rule_arg)
            ...
            if rule == 'Dirichlet':
                if os.path.exists('%sData/%s/clnt_x.npy' % (self.data_path, self.name)):
...
def split_datasets(dataname, num_clients, num_class, seed, rule, alpha, data_path='./data', showfig=False):
